...
public class RealtimeDao {
     ...
     * @return
     */
    public List<Entity> listTopTen(int week,int hour){
        String sql ="select count(*),start_station_id FROM t_trip_202006 where " +
                "dayofweek(STR_TO_DATE(SUBSTRING(starttime FROM 1 FOR 10) , '%Y-%m-%d'))=? " +
                "and SUBSTRING(starttime FROM 12 FOR 2)=? GROUP BY start_station_id order by count(*) desc limit 10;";
        List<Entity> list = null;
        try {
            list = Db.use().query(sql,week,hour);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return list;
